...
*/		final float lower_bound = 1;
		...
		for(int i=0; i<slabels.length; i++){
			...
			if(slabels[i].contains(":")){
				...
				labels[i].low = processNumbers(compare_pro, labels[i].low);
				labels[i].high = Float.parseFloat(s[1]);
				labels[i].high = processNumbers(compare_pro, labels[i].high);
			}
...
//			System.out.println(labels[i].low+" "+labels[i].high);
		...
		final String[] filenames = {
				"C:/Users/testuser/SkyDrive/Documents/4th year project files/repos/4th-year-project/24th Sept ORDERED.csv",
				"C:/Users/testuser/SkyDrive/Documents/4th year project files/repos/4th-year-project/26th Sept ORDERED.csv"
		};
		...
		for(int g=0; g<filenames.length; g++){
			DataGetter dg = new DataGetter(1, filenames[g]);
			for(int h=1; h<=5; h++){
				if(h>1){
					dg.changePhone(h);
				}
...
//					System.out.println(result[g*5+h-1][i] +" "+denominator);
					if (within_bounds)
