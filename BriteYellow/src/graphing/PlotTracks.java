...
public class PlotTracks {
	...
	public static void plotTrack2(final PhoneData[] before,final PhoneData[] after, final int row, final int col, final float timescale){
		...
		final String PLAY = ""+(char)9658+" Playing at "+(1/timescale)+"X speed";
		final String STOP = (char)9632+" Stopped";
		final String PAUSE1 = ""+(char)73+(char)73+" Paused";
		final String PAUSE2 = "<html>"+(char)73+(char)73+" Paused <i>(Release RMB to resume)</i></html>";

		final JButton jlabel1 = new JButton(PLAY);	// Label for "Playing at XX Speed"
		...
		c.gridy = 0;
		c.weightx = 0;
		subpanel1.add(jlabel1,c);
		c.gridx = 1;
		c.weightx = 1;
		subpanel1.add(new JPanel(),c);
		...
		final TimeLine tl = new TimeLine(before, after, (int)(100/timescale), tel, row, col, label[0], label[1], label[2], label[3]);
		...
		jlabel1.setMargin(new Insets(0, 10, 0, 10));
		...
		jpb.addMouseMotionListener(new MouseMotionListener(){
	...
	public static void plotTrack3(final PhoneData[] track_info, final int row, final int col, final float pointspersec){
		...
		final String PLAY = ""+(char)9658+" Showing at "+(1/pointspersec)+" point(s) / second";
		final String STOP = (char)9632+" Stopped";
		...
		final TimeLine tl = new TimeLine(track_info, null, 1000, tel, row, col, label[0], null, null, null);
		// Get and configure the plot
		final PlotHelper plot = tl.getPlot();
		plot.setAxisRange(0, 1100, 0, 500);
		plot.setRangeAxisInverted(true);
...
class TimeLine{
	...
	public TimeLine(final PhoneData[] before,final PhoneData[] after, int interval, TimerEventsListener etel, int row, int col, String beforeseries, String afterseries, String beforelast, String afterlast){
		// Conditions that are not allowed
		if(before==null || beforeseries==null || (after!=null && afterseries==null) || (after==null&&(afterseries!=null||afterlast!=null)))
		...
		plot = new PlotHelper(PlotTracks.COLUMNS[row]+" vs "+PlotTracks.COLUMNS[col],PlotTracks.COLUMNS[row], PlotTracks.COLUMNS[col], plotlabels);
		this.before = before;
		this.after = after;
	...
	private synchronized void setCurrentTime(int index, float percent){
		...
		if(index == getCurrentPoint() && previous_time == current_time)
			return;
		if(!finished){
	...
	public synchronized void advanceTime(boolean once){
		if(! finished){
			...
			if(after != null){
				...
				while((iafter<after.length && (current_time - after[iafter].ts.getTime())>=0)|| iafter == after.length){
